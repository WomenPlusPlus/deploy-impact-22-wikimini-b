...
export const doTeacherSignUp = async (req, res) => {
  try {
    ...
    if (
      teacherSignUpResult[createAccountAction]["status"] === "PASS" &&
      emailSent
    ) {
      ...
      res.status(405).json({
        success: false,
        res: teacherSignUpResult[createAccountAction],
      });
    }
  } catch (error) {
...
export const doTeacherLogin = async (req, res) => {
  try {
    ...
    if (userInfo["isVerified"] === 0) {
      throw Error(
        "The user did not verify their email address, not proceeding with login"
      );
    }
...
export async function sendTeacherConfirmationMail(username, email) {
  ...
  const authCodeRegistered = await dbAdapter.registerTeacherAuthCode(
    username,
    authCode
  );
  ...
  await wikiAdapter
    ...
    .then((response) => {
      if (response.userrights.added[0] === teacherUserGroup) {
        ...
        throw Error(
          "Error while trying to register the account as a teacher's account: " +
            response.userrights
        ); // should have a nicer format
      }
    })
    .catch((e) => {
      throw Error(
...
export async function sendStudentCodesForParents(username, emails) {
  const list = await dbAdapter.getStudentCodesForTeacher(username);
  const subject = "Account signup code";
  for (let i = 0; i < list.length; i++) {
    if (emails.includes(list[i].parentEmail)) {
      await emailAdapter.sendEmail(
        list[i].parentEmail,
        subject,
        generateMessage(list[i].studentJoinCode)
      );
    }
  }
  return true;
...
export const shareCodesWithParents = async (req, res) => {
  try {
    const { username, emails } = req.body;
    const emailsSent = await sendStudentCodesForParents(username, emails);
    res.status(200).json({ success: true, res: "emails Sent" });
  } catch (error) {
    res.status(405).json({ message: error.message });
  }
};
